...
Exports are configured **per data source** (per region/Project). To export data from several regions or Projects, you must create one export per data source. You can also create multiple exports per data source if you want to send data to different [export destinations](#export-destination).
