...
2. Add the following code to your Python script:
    ...
    client = OpenAI(
        base_url="https://api.scaleway.ai/<SCw_PROJECT_ID>/v1", # Scaleway's Generative APIs service URL
        api_key="<SCW_SECRET_KEY>" # Your unique API secret key from Scaleway
    )
    ```

    Make sure that you replace `<SCW_PROJECT_ID>` with the identifier of the Scaleway Project and `<SCW_SECRET_KEY>` with the API key obtained from your
    Scaleway account.
